...
# Script para configurar worktree do Flutter para Claude Code
...
# Tentar detectar o diretório raiz do worktree original
# ROOT_WORKTREE_PATH pode ser passado como argumento ou variável de ambiente
if [ -z "$ROOT_WORKTREE_PATH" ]; then
  if [ -n "$1" ]; then
    ROOT_WORKTREE_PATH="$1"
  else
    # Tentar detectar automaticamente via git
    ROOT_WORKTREE_PATH=$(git worktree list | head -1 | awk '{print $1}')
  fi
fi

if [ -z "$ROOT_WORKTREE_PATH" ] || [ ! -d "$ROOT_WORKTREE_PATH" ]; then
  echo "⚠️  AVISO: ROOT_WORKTREE_PATH não está definido ou não existe"
  echo "   Uso: $0 /caminho/para/projeto/original"
  echo ""
fi
...
# Copiar arquivos de configuração se ROOT_WORKTREE_PATH estiver definido
if [ -n "$ROOT_WORKTREE_PATH" ] && [ -d "$ROOT_WORKTREE_PATH" ]; then

  # Copiar .fvmrc se não existir no worktree
