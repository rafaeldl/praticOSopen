...
import { validateInput, optionalSearchQuerySchema } from '../../utils/validation.utils';
...
router.get('/search', requireLinked, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const validation = validateInput(optionalSearchQuerySchema, req.query);
    ...
    if (q) {
      ...
      const all = await customerService.searchCustomers(companyId, '', 20);
...
router.get('/devices/search', requireLinked, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (q) {
      ...
      // List all devices (limited)
      const all = await deviceService.searchDevices(companyId, '', 20);
      suggestions = all.map((d) => ({
        id: d.id,
        name: d.name,
        serial: d.serial ?? undefined,
      }));
